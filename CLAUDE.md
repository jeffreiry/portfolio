...
**Imagens dos cases (2026-06-20):** Imagens reais adicionadas em `public/cases/{slug}/` para arezzo-ad-management, del-valle-kapo, del-valle-website, enterprise-ai-assistant, shipping-capacity-platform e power-apps-dummy-app. Convenção: `visao-geral.*` como capa + arquivos numerados `01-*.png`, `02-*.png`, `03-*.png` para screenshots inline no body do case. `src/utils/cover.ts` — utilitário `detectCover(slug)` que detecta automaticamente `visao-geral.*` ou `visao_geral.*` (png/jpg/jpeg/webp) em `public/cases/{slug}/` usando `fs.existsSync`, sem exigir `cover:` no frontmatter. Usado em `index.astro` (EN+PT) via `cover={detectCover(c.data.slug) ?? c.data.cover}` no CaseCard, e em `work/[slug].astro` (EN+PT) como `const cover = detectCover(slug!) ?? entry.data.cover`. Placeholders `<div class="image-placeholder">` substituídos por `![alt](path)` nos cases com screenshots disponíveis (PT+EN). Extensões `.jpg` corrigidas para `.png` nos frontmatters de arezzo, del-valle-kapo, del-valle-website e power-apps-dummy-app.
