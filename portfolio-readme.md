...
**Bench de vagas (2026-09-12 · 55 vagas · score médio 57%):** Análises em `Bench_job_applications/`, método Person-Job Fit ponderado. Pipeline de análise em dois passos: Groq (`openai/gpt-oss-120b`) extrai estrutura da JD em JSON, Claude Sonnet (`claude-sonnet-4-6`) raciocina sobre aderência ao portfolio com contexto completo dos 9 cases. Score e subtotais recalculados deterministicamente no servidor. "Atualizado em" na página dinâmico (lê maior `data` dos arquivos .md). A seção "Gaps transversais" também é lida dinamicamente do `_index.md` (sem array hardcoded — fix 2026-09-12). Escrita de arquivo só funciona rodando local — em produção a página `/jobanalysis` é só-visualização (sem os botões Nova Vaga/Editar/Candidatura).
